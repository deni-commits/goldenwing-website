          ...
          {block.packages?.map((pkg, i) => (
            <div
              ...
              className={`relative rounded-2xl border p-8 ${
                pkg.highlighted ? 'border-primary bg-primary/5 shadow-primary/10 shadow-lg' : 'border-border bg-card'
              }`}
            >
              {pkg.highlighted && (
              ...
              <div className="mb-6">
                ...
                {pkg.period && <span className="text-muted-foreground ml-1 text-sm">/{pkg.period}</span>}
              </div>
