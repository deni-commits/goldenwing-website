      ...
      <div className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid gap-8 md:grid-cols-4">
          <div>
            ...
            <p className="mt-4 text-sm">{t.footer.agency}</p>
            {social && (social.linkedin || social.instagram) && (
              <div className="mt-4 flex gap-3">
                {social.linkedin && (
                  <a href={social.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="text-gray-400 transition hover:text-gold-400">
                ...
                {social.instagram && (
                  <a href={social.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-gray-400 transition hover:text-gold-400">
                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {cmsColumns && cmsColumns.length > 0 ? (
              ...
              <div key={i}>
                ...
                <ul className="space-y-2 text-sm">
                  {(column.links as any[] || []).map((link: any, j: number) => (
                    <li key={j}>
                      {link.newTab ? (
                        <a href={link.link as string} target="_blank" rel="noopener noreferrer" className="hover:text-gold-400">
                          {link.label as string}
                        </a>
                      ) : (
                        <Link href={link.link as string} className="hover:text-gold-400">
                          {link.label as string}
                        </Link>
                      )}
                    </li>
            ...
            <>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li><Link href={`/${locale}/ueber-uns`} className="hover:text-gold-400">{t.nav.about}</Link></li>
                  <li><Link href={`/${locale}/referenzen`} className="hover:text-gold-400">{t.nav.referenzen}</Link></li>
                  <li><Link href={`/${locale}/blog`} className="hover:text-gold-400">{t.nav.blog || 'Blog'}</Link></li>
                  <li><Link href={`/${locale}/kontakt`} className="hover:text-gold-400">{t.nav.contact}</Link></li>
                </ul>
              </div>
            </>
          )}
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li>
                <a href={`mailto:${contactEmail}`} className="hover:text-gold-400">
                  {contactEmail}
                </a>
              </li>
              {siteSettings?.contact?.phone && (
                <li>
                  <a href={`tel:${siteSettings.contact.phone as string}`} className="hover:text-gold-400">
          ...
          <div className="mt-4 flex gap-6 md:mt-0">
            {cmsLegalLinks && cmsLegalLinks.length > 0 ? (
              cmsLegalLinks.map((link: any, i: number) => (
                <Link key={i} href={link.link as string} className="hover:text-gold-400">
                  {link.label as string}
                </Link>
              ))
            ) : (
              <>
                <Link href={`/${locale}/impressum`} className="hover:text-gold-400">{t.nav.impressum}</Link>
                <Link href={`/${locale}/datenschutz`} className="hover:text-gold-400">{t.nav.datenschutz}</Link>
                <Link href={`/${locale}/agb`} className="hover:text-gold-400">{t.nav.agb}</Link>
              </>
            )}
          </div>
        </div>
