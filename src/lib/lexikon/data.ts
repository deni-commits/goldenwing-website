...
export interface LexikonEntry {
  ...
  de: LexikonContent
  en: LexikonContent
  ru?: LexikonContent // Optional - falls back to English
...
// Get lexikon entries that match relatedTerms text
export function getLinkedRelatedTerms(slug: string, locale: 'de' | 'en' | 'ru'): { term: string; slug: string; hasLink: boolean }[] {
  const entry = getLexikonEntry(slug)
  if (!entry) return []
  ...
  return relatedTerms.map(term => {
    // Try to find a matching lexikon entry by comparing term names
    const termLower = term.toLowerCase()
    const matchingEntry = lexikonEntries.find(e => {
      if (e.slug === slug) return false // Don't link to self
      const deTerm = e.de.term.toLowerCase()
      const enTerm = e.en.term.toLowerCase()
      // Check if term matches the entry's term (with or without parenthetical)
      return deTerm === termLower ||
             enTerm === termLower ||
             deTerm.split(' (')[0].toLowerCase() === termLower ||
             enTerm.split(' (')[0].toLowerCase() === termLower ||
             e.slug === termLower.replace(/[^a-z0-9]/g, '-')
    })

    return {
      term,
      slug: matchingEntry?.slug || '',
      hasLink: !!matchingEntry
    }
  })
}

// Get cross-category related entries for more internal links
export function getCrossCategoryRelatedEntries(slug: string, limit: number = 5): LexikonEntry[] {
  const entry = getLexikonEntry(slug)
  if (!entry) return []

  // Get entries from OTHER categories (cross-linking)
  return lexikonEntries
    .filter(e => e.slug !== slug && e.category !== entry.category)
...
export const lexikonCategories = {
  ...
  ru: {
    seo: 'SEO и поисковые системы',
    marketing: 'Маркетинг и стратегия',
    design: 'Дизайн и UX',
    development: 'Разработка и технологии',
    strategy: 'Стратегия и брендинг'
  }
}
