      ...
      <article className="px-4 py-24">
        <div className="mx-auto max-w-4xl">
          <nav className="text-muted-foreground mb-6 text-sm">
            ...
            {' / '}
            <Link href={`/${locale}/referenzen`} className="hover:text-primary">
              {t.nav.referenzen}
            </Link>
            {' / '}
            <span className="text-foreground">{caseStudy.title as string}</span>
          </nav>

          <MotionSection>
            <h1 className="mb-4 text-4xl font-bold md:text-5xl">{caseStudy.title as string}</h1>
            {caseStudy.client && <p className="text-muted-foreground mb-8 text-lg">{caseStudy.client as string}</p>}
          </MotionSection>

          {/* Results Metrics */}
          {results.length > 0 && (
            <MotionSection as="div" stagger={0.1} className="mb-16 grid gap-6 md:grid-cols-3">
              {results.map((r: any, i: number) => (
                <div key={i} className="border-primary/20 bg-primary/10 rounded-xl border p-6 text-center">
                  <p className="text-primary text-3xl font-bold">{r.value as string}</p>
          ...
          {/* Challenge */}
          {caseStudy.challenge && (
            <MotionSection as="section" className="mb-12">
